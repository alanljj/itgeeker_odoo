        ...
        if self.cv:
            ...
            for industry in industries:
                ...
                if not self.industyry:
                    self.industyry = self.env['geeker.industry'].create(
                        {'name': industries[0]})
            ...
            for e in emails:
                ...
                if existed_email:
                    ...
                    url = str(base_url) + "/web#id=" + str(partner_email_obj.id) + \
                        "&view_type=form&model=res.partner&menu_id=722&action=741"
                    self.duplicated_url = url
            ...
            # first link
            links = self.env[
                "ir.fields.converter"].text_from_html_link(self.cv)
            for link in links:
